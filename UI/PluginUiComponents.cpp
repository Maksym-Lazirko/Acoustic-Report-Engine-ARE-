...
void SpectrumPlotPanel::paint (juce::Graphics& g)
{
    ...
    g.setFont (juce::FontOptions().withHeight (11.0f));
    g.drawText ("Average spectral balance (Welch PSD, dB/Hz)", outer.removeFromTop (18.0f).toNearestInt(), juce::Justification::centredLeft);
...
void DynamicsPlotPanel::paint (juce::Graphics& g)
{
    ...
    g.setColour (juce::Colour (0xffe8eaed));
    g.setFont (juce::FontOptions().withHeight (11.0f));
    g.drawText ("RMS loudness envelope (dBFS) vs time", outer.removeFromTop (20.0f).toNearestInt(), juce::Justification::centredLeft);
    ...
    : owner (editor)
{
    ...
    titleLabel.setText ("Load reference audio to compare", juce::dontSendNotification);
