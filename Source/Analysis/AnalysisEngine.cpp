                                      ...
                                      std::vector<float>& monoOut)
{
    ...
    const int toCopy = juce::jmax (0, juce::jmin (numSamples, avail));
    ...
    // Copy available frames and fill any tail with zeros if caller requested more samples
...
{
    if (waveformBuckets <= 0)
    {
        peaksOut.clear();
        return;
    }
    areDbgLog ("H1", "decimateWaveformPeaks", "entry", (int) mono.size(), numSamples);
...
void AnalysisEngine::measureVu (const std::vector<float>& mono,
                                ...
                                double& outMax)
{
    areDbgLog ("H1", "measureVu", "entry", (int) mono.size(), (int) std::llround (sampleRate));
    ...
    const size_t winLenU = (size_t) winLen;
    for (size_t i = 0; i < mono.size(); ++i)
    {
        ...
        if (i >= winLenU)
        {
            const double lv = (double) mono[i - winLenU];
            sumSq -= lv * lv;
        }
...
{
    areDbgLog ("H1", "measureWelchSpectrum", "entry", (int) mono.size(), (int) std::llround (sampleRate));
...
{
    areDbgLog ("H1", "measureRmsEnvelope", "entry", (int) mono.size(), (int) std::llround (sampleRate));
                                                ...
                                                const juce::String& label)
{
    ...
    // Extra defensive logging to help diagnose any remaining vector OOB in debug runs.
    areDbgLog ("H1", "analyzePlanar", "buffer_info", buffer.getNumSamples(), snap.numChannels);

    std::vector<float> mono;
    downmixMonoMean (buffer, numSamples, mono);

    // Ensure mono length matches numSamples. If downmix produced fewer
    // samples due to an unexpected buffer size, pad with zeros to avoid
    // confusing envelope/spectrum time axes (tMax) later on.
    if ((int) mono.size() < numSamples)
    {
        const size_t old = mono.size();
        try
        {
            mono.resize ((size_t) numSamples, 0.0f);
        }
        catch (...) { /* ignore resize failure - we'll proceed with smaller vector */ }
        areDbgLog ("H1", "analyzePlanar", "mono_padded", (int) old, numSamples);
    }
