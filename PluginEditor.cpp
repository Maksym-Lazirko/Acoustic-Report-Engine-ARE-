...
public:
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        else
        {
            ...
            text << juce::String (" ").paddedRight (' ', 22) + "(LUFS: multichannel BS.1770/EBU R128 meter)\n";
    ...
    , audioProcessor (p)
{
    ...
    realtimeToggle.setToggleState (audioProcessor.isRealtimeMonitoringEnabled(), juce::dontSendNotification);
    realtimeToggle.addListener (this);
    ...
    {
        juce::String s;
        s << "VST3 v" << JucePlugin_VersionString << " | " << __DATE__ << " by Maksym Lazirko";
...
void AREPluginEditor::resized()
{
    ...
    realtimeToggle.setBounds (row.removeFromLeft (188).reduced (0, 3));
...
void AREPluginEditor::buttonClicked (juce::Button* b)
{
    ...
    else if (b == &realtimeToggle)
    {
        const bool v = realtimeToggle.getToggleState();
        audioProcessor.setRealtimeMonitoringEnabled (v);
        areDbgLog ("H2", "realtimeToggle", v ? "enabled" : "disabled", (int) v, 0);
        // If enabling realtime monitoring, trigger live capture analysis immediately
        if (v)
        {
            // Start analysis of live capture regardless of ARA binding when toggle on
            audioProcessor.beginTimelineAnalysis (getARAEditorView());
            refreshUi();
        }
    }
